...
static int load_media()
{
	...
	ret = load_from_rendered_text(
		&text_texture, "Press 'Enter' to reset timer", textColor);
	if (ret < 0)
...
int main()
{
	...
	while (!quit) {
		...
		// set text to render
		snprintf(time_str, 64, "Milliseconds since start time = %u",
		...
		// render textures
		render(&text_texture, (SCREEN_WIDTH - text_texture.width) / 2,
		       0, NULL);
